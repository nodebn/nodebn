...
export function ProductGrid({
  ...
  return (
    <div className="space-y-8">
      ...
      {/* Show uncategorized products at the bottom */}
      {groupedProducts["Uncategorized"] && groupedProducts["Uncategorized"].length > 0 && (
        <Card className="bg-amber-50 border-amber-200 rounded-2xl p-6 space-y-4 group">
          <div className="flex items-center gap-2">
            <Package className="h-5 w-5 text-amber-600" />
            <h3 className="font-bold text-lg text-amber-800">Uncategorized Products</h3>
          </div>
          <p className="text-sm text-amber-700">
            These products haven't been organized into categories yet. Check back soon for better organization!
          </p>
          <ul className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {groupedProducts["Uncategorized"]
              .slice(0, isCategoryPage ? undefined : 8)
              .map((product) => {
                const src = primaryImage(product);
                const hasVariants = product.product_variants.length > 0;
                const displayPrice = hasVariants
                  ? Math.min(...product.product_variants.filter(v => v.is_active).map(v => v.price_cents))
                  : product.price_cents;

                return (
                  <li key={product.id} className="group/product">
                    <Card className="bg-white border-gray-200 rounded-xl overflow-hidden hover:shadow-md transition-all duration-200 hover:border-gray-300">
                      <CardContent className="p-0">
                        ...
                        <div className="space-y-1.5 px-3 pt-3 sm:px-4 sm:pt-4">
                          <Badge
                            variant="secondary"
                            className="w-fit max-w-full truncate border-0 bg-amber-100 font-medium text-amber-700"
                          >
                            Not Categorized
                          </Badge>
                          <CardTitle className="line-clamp-2 text-[0.9375rem] font-semibold leading-snug sm:text-base">
